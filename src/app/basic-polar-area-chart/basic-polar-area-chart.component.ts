...
import { ChartComponent, ApexChart, ApexNonAxisChartSeries, ApexResponsive, ApexLegend } from 'ng-apexcharts';
...
@Component({
  selector: 'app-basic-polar-area-chart',
  templateUrl: './basic-polar-area-chart.component.html',
  styleUrls: ['./basic-polar-area-chart.component.scss']
})
export class BasicPolarAreaChartComponent implements OnInit {
  ...
  updateChart() {
    const filteredData = this.Tesellum.filter(item => item.DonemYili === this.selectedYear);

    const seriesData = filteredData.map(item => item.NetPancar);
    const labelsData = filteredData.map(item => item.FABRIKA_ADI);
